...
describe("protected API routes", () => {
    ...
    it("returns 400 for invalid reading progress payload", async () => {
        ...
        const response = await route.POST(makeJsonRequest("http://localhost/api/reading-progress", "POST", { textId: "", lastVerseIndex: "x" }));
        expect(response.status).toBe(400);
    ...
    it("appends content to ShastraBook", async () => {
        ...
        const route = await import("@/app/api/shastrabooks/[id]/append/route");
        const response = await route.POST(makeJsonRequest("http://localhost/api/shastrabooks/nb1/append", "POST", { content: "New block" }), { params: { id: "nb1" } });
        const payload = await response.json();
        expect(response.status).toBe(200);
    ...
    it("creates chat thread message with valid payload", async () => {
        ...
        authContext.supabase = {
            from: vi.fn((table) => table === "chat_messages"
                ? { insert }
                : { update: vi.fn(() => updateEq) }),
        };
        const route = await import("@/app/api/chat/threads/[id]/route");
        const response = await route.POST(makeJsonRequest("http://localhost/api/chat/threads/thread-1", "POST", { role: "user", content: "Hi" }), { params: { id: "thread-1" } });
        const payload = await response.json();
        expect(response.status).toBe(200);
    ...
    it("renames chat thread with valid payload", async () => {
        ...
        const route = await import("@/app/api/chat/threads/[id]/route");
        const response = await route.PATCH(makeJsonRequest("http://localhost/api/chat/threads/thread-1", "PATCH", { title: "Renamed thread" }), { params: { id: "thread-1" } });
        const payload = await response.json();
        expect(response.status).toBe(200);
