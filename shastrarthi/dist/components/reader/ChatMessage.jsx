...
export default function ChatMessage({ role, content, timestamp }) {
    ...
    return (<div className={cn("flex gap-3 max-w-[85%]", isUser ? "flex-row-reverse" : "flex-row")}>
            {/* Avatar */}
            ...
            : "bg-sand-200 dark:bg-sand-700 text-sand-700 dark:text-sand-300")}>
                {isUser ? (<User className="h-5 w-5"/>) : (<Bot className="h-5 w-5 text-sand-600 dark:text-sand-400"/>)}
            </div>

            {/* Message Content */}
            <div className={cn("flex-1 rounded-2xl px-4 py-3", isUser
