...
export default function QuickChatPrompts({ onAction }) {
    return (<div className="space-y-2">
            <p className="text-sm font-medium text-sand-700 dark:text-sand-300 mb-3">
                Quick questions:
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {QUICK_ACTIONS.map((action) => (<button key={action.id} onClick={() => onAction(action.prompt)} className={cn("flex items-center gap-2 p-3 rounded-lg border border-sand-200 dark:border-sand-700", "bg-white dark:bg-sand-800 hover:border-saffron-300 dark:hover:border-saffron-700", "transition-all duration-200 text-left")}>
