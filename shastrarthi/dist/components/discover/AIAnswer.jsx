...
export default function AIAnswer({ query, texts, contextLabel }) {
    ...
    const [availableShastraBooks, setAvailableShastraBooks] = useState([]);
    ...
    const loadShastraBooks = async () => {
        ...
        const shastraBooks = ((_a = payload.data) !== null && _a !== void 0 ? _a : []);
    ...
    const handleSaveToShastraBook = async () => {
        ...
        try {
            ...
            if (saveTarget === "new") {
                ...
                targetShastraBookId = createPayload.data.id;
            }
            ...
            if (!appendResponse.ok) {
                ...
                throw new Error((_a = appendPayload.error) !== null && _a !== void 0 ? _a : "Failed to save synthesis");
            }
            await loadShastraBooks();
    ...
    return (<section className="bg-white border border-gray-200 rounded-xl shadow-sm p-5">
            <p className="text-sm text-gray-500 mb-3">
                {contextLabel !== null && contextLabel !== void 0 ? contextLabel : (<>
            ...
            {isLoading && !response ? (<p className="text-sm text-gray-600">Generating synthesis...</p>) : null}
            {error ? (<p className="text-sm text-red-600">{error}</p>) : (<div className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
                <button onClick={() => void handleOpenSavePanel()} className="inline-flex items-center gap-1.5 px-3 py-1.5 border border-gray-200 rounded-md text-sm text-gray-600 hover:bg-gray-50">
                    <BookmarkPlus className="h-4 w-4"/>
                    Save to ShastraBook
                </button>
                <button onClick={() => navigator.clipboard.writeText(response || "")} className="inline-flex items-center gap-1.5 px-3 py-1.5 border border-gray-200 rounded-md text-sm text-gray-600 hover:bg-gray-50">
            ...
            {isSavePanelOpen ? (<div className="mt-4 rounded-lg border border-gray-200 bg-gray-50 p-3 space-y-3">
                    <h3 className="text-sm font-medium text-gray-900">Save synthesis</h3>
                    <label className="block text-xs text-gray-600">
                        Save destination
                        <select value={saveTarget} onChange={(event) => setSaveTarget(event.target.value)} className="mt-1 w-full rounded-md border border-gray-200 bg-white px-2 py-1.5 text-sm text-gray-700">
                            {availableShastraBooks.map((shastraBook) => (<option key={shastraBook.id} value={shastraBook.id}>
                                    {shastraBook.title}
                                </option>))}
                            <option value="new">+ Create new ShastraBook</option>
                        </select>
                    </label>
                    {saveTarget === "new" ? (<label className="block text-xs text-gray-600">
                            New ShastraBook title
                            <input value={newShastraBookTitle} onChange={(event) => setNewShastraBookTitle(event.target.value)} className="mt-1 w-full rounded-md border border-gray-200 bg-white px-2 py-1.5 text-sm text-gray-700"/>
                        </label>) : null}
                    <div className="flex items-center gap-2">
                        <button onClick={() => void handleSaveToShastraBook()} disabled={isSavingToShastraBook} className="rounded-md bg-orange-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-orange-700 disabled:opacity-60">
                            {isSavingToShastraBook ? "Saving..." : "Save now"}
                        </button>
                        <button onClick={() => setIsSavePanelOpen(false)} className="rounded-md border border-gray-200 px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-100">
                            Cancel
                        </button>
                    </div>
