...
export default function CustomColumnPanel({ mode = "all" }) {
    var _a;
    const suggested = (_a = SUGGESTED_BY_MODE[mode]) !== null && _a !== void 0 ? _a : SUGGESTED_BY_MODE.all;
    return (<aside className="bg-white border border-gray-200 rounded-xl shadow-sm p-4">
            ...
            <div className="space-y-2">
                {suggested.map((item) => (<button key={item} className="w-full text-left px-3 py-2 rounded-md border border-gray-200 text-sm text-gray-600 hover:bg-gray-50">
                        + {item}
                    </button>))}
            </div>
        </aside>);
