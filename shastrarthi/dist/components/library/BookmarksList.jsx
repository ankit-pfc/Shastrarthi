...
export default function BookmarksList({ bookmarks, onRemove }) {
    return (<div className="space-y-3">
            ...
            {bookmarks.length === 0 ? (<div className="text-center py-16">
                    <p className="text-sand-600 dark:text-sand-400 text-lg">
                        No bookmarks yet
                    </p>
                </div>) : (<div className="space-y-3">
                    {bookmarks.map((bookmark) => (<div key={bookmark.id} className="bg-white dark:bg-sand-800 rounded-lg shadow-sm border border-sand-200 dark:border-sand-700 p-4">
                            <div className="flex items-start justify-between gap-3 mb-2">
                                <div className="flex items-center gap-2">
                                    <Bookmark className="h-5 w-5 text-saffron-600 dark:text-saffron-400"/>
                                    <div className="flex-1 min-w-0">
                                        <Link href={`/app/reader/${bookmark.textSlug}`} className="group">
                                ...
                                <button onClick={async () => await onRemove(bookmark.id)} className="p-2 rounded-md text-sand-500 hover:bg-red-50 dark:text-sand-400 dark:hover:bg-red-900/20 dark:hover:text-red-300 transition-colors" aria-label="Remove bookmark">
