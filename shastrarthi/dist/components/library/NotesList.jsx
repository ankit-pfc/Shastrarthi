...
export default function NotesList({ notes, onRemove }) {
    return (<div className="space-y-3">
            ...
            {notes.length === 0 ? (<div className="text-center py-16">
                    ...
                    {notes.map((note) => (<div key={note.id} className="bg-white dark:bg-sand-800 rounded-lg shadow-sm border border-sand-200 dark:border-sand-700 p-4">
                            <div className="flex items-start justify-between gap-3 mb-3">
                                <div className="flex items-center gap-2">
                                    <FileText className="h-5 w-5 text-saffron-600 dark:text-saffron-400"/>
                                    <div className="flex-1 min-w-0">
                                        <Link href={`/app/reader/${note.textSlug}`} className="group">
                                ...
                                <button onClick={async () => await onRemove(note.id)} className="p-2 rounded-md text-sand-500 hover:bg-red-50 dark:text-sand-400 dark:hover:bg-red-900/20 dark:hover:text-red-300 transition-colors" aria-label="Remove note">
                            ...
                            <div className="flex items-center justify-between text-xs text-sand-500 dark:text-sand-500">
                                ...
                                <Link href={`/app/reader/${note.textSlug}`} className="text-saffron-600 dark:text-saffron-400 hover:underline">
