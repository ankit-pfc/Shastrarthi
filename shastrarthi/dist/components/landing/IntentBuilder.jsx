...
export default function IntentBuilder({ compact = false, routePrefix = "/app/discover" }) {
    ...
    const toggleColumnExpansion = (columnIndex) => {
        const keys = ["goals", "lenses", "starts"];
        const key = keys[columnIndex];
        setExpandedColumns((prev) => (Object.assign(Object.assign({}, prev), { [key]: !prev[key] })));
    };
    const handleItemClick = (columnIndex, itemId, isShowMore) => {
        if (isShowMore) {
    ...
    const getSelectedState = (columnIndex, itemValue) => {
        return ((columnIndex === 0 && selectedGoal === itemValue) ||
            (columnIndex === 1 && selectedLens === itemValue) ||
            (columnIndex === 2 && selectedStart === itemValue));
    };
    return (<section className={cn("bg-gray-50", compact ? "py-8" : "py-16")}>
            ...
            return (<div key={column.title} className="space-y-2 bg-gray-50 rounded-xl p-3 border border-gray-100">
                    ...
                    return (<button key={item.id} type="button" onClick={() => handleItemClick(columnIndex, item.value, item.isShowMore || false)} className={cn("w-full text-left px-3 py-2 rounded-lg transition-all border", "bg-white border-gray-100 hover:border-gray-200 hover:shadow-sm", isSelected && "border-orange-300 bg-orange-50")}>
                                        ...
                                        {expandedItems.length > 0 && (<button type="button" onClick={() => toggleColumnExpansion(columnIndex)} className="w-full py-1.5 text-xs font-medium text-gray-400 hover:text-gray-600 transition-colors">
