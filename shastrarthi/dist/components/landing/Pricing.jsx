    ...
    return (<section className="bg-white py-20">
            <div className="container mx-auto px-4">
                ...
                <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-5">
                    {TIERS.map((tier) => (<div key={tier.name} className={`rounded-2xl border p-6 shadow-sm ${tier.featured ? "border-orange-300 bg-orange-50/40" : "border-gray-200 bg-white"}`}>
                            ...
                            <Link href="/auth/signup" className={`mt-6 inline-flex w-full justify-center rounded-lg px-4 py-2.5 font-medium transition-colors ${tier.featured ? "bg-orange-600 hover:bg-orange-700 text-white" : "bg-gray-900 hover:bg-gray-800 text-white"}`}>
                                Get Started
                            </Link>
                        </div>))}
                </div>
