...
export default function Hero() {
    ...
    return (<div className="relative overflow-hidden bg-[radial-gradient(ellipse_at_top_left,_var(--tw-gradient-stops))] from-white via-white to-orange-100/40">
                ...
                <div className="flex flex-col items-center text-center">
                    ...
                    <div className="mb-10 w-full">
                        <div className="flex flex-wrap justify-center gap-2 mb-4">
                            {Object.keys(modeConfig).map((key) => (<button key={key} type="button" onClick={() => setMode(key)} className={cn("px-3 py-1.5 rounded-full text-xs font-semibold border transition-colors", mode === key
                ? "bg-orange-600 text-white border-orange-600"
                : "bg-white text-gray-700 border-gray-200 hover:bg-gray-50")} aria-pressed={mode === key}>
                                    {modeConfig[key].label}
                                </button>))}
                        </div>
                        <SearchInput placeholder={modeConfig[mode].placeholder} helperText={modeConfig[mode].helperText} onSearch={handleSearch}/>
                    </div>
