...
export default function Breadcrumb() {
    ...
    const parts = (pathname === null || pathname === void 0 ? void 0 : pathname.split("/").filter(Boolean)) || [];
    const appIndex = parts.findIndex((part) => part === "app");
    const scoped = appIndex >= 0 ? parts.slice(appIndex) : parts;
    return (<nav className="flex items-center gap-1 text-sm text-gray-500">
            {scoped.map((part, idx) => {
            ...
            return (<span key={href} className="inline-flex items-center gap-1">
                        {idx > 0 && <ChevronRight className="h-3.5 w-3.5"/>}
                        {isLast ? (<span className="text-gray-700 capitalize">{label}</span>) : (<Link href={href} className="hover:text-gray-700 capitalize">
                                {label}
                            </Link>)}
                    </span>);
        })}
