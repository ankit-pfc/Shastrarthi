...
export default function GalleryPage() {
    return (<div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {AGENTS.map((agent) => (<div key={agent.name} className="bg-white border border-gray-200 rounded-xl shadow-sm p-4">
                        <h3 className="font-semibold text-gray-900">{agent.name}</h3>
                        <p className="text-sm text-gray-600 mt-2">{agent.desc}</p>
                        <Link href={`/app/chat?agent=${agent.key}`} className="inline-block mt-4 px-3 py-2 rounded-md bg-orange-600 hover:bg-orange-700 text-white text-sm">
                            Start Chat
                        </Link>
                    </div>))}
            </div>
