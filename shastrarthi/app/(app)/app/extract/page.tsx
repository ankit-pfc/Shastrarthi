...
import { Text, ExtractDataset } from "@/lib/supabase";
...
export default function ExtractPage() {
    ...
    const [extractedRows, setExtractedRows] = useState<ExtractedInsight[]>([]);
    const [isLoading, setIsLoading] = useState(false);
    const [availableTexts, setAvailableTexts] = useState<Text[]>([]);
    const [availableDatasets, setAvailableDatasets] = useState<ExtractDataset[]>([]);
        ...
        <div className="space-y-4">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
                ...
                <input
                    ...
                    placeholder="E.g., How is detachment explained?"
                ...
                <div className="space-y-2">
                    <MultiSelect
                        options={textOptions}
                        selectedValues={selectedTextIds}
                        onSelectChange={setSelectedTextIds}
                        placeholder="Select Shastra texts..."
                    />
                </div>
                ...
                <div className="border-t border-gray-100 pt-3 mt-3 space-y-2">
                    ...
                    <div className="flex items-center space-x-2">
                        <input
                            type="text"
                            placeholder="Dataset Name (e.g., My Research Notes)"
                            value={newDatasetName}
                            onChange={(e) => setNewDatasetName(e.target.value)}
                            className="flex-grow rounded-lg border border-gray-200 px-3 py-2 outline-none focus:ring-2 focus:ring-orange-500"
                        />
                        ...
                        <button
                            ...
                            disabled={isLoading || !newDatasetFile || !newDatasetName.trim()}
                            className="px-4 py-2 rounded-md bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white text-sm font-medium"
                        >
                            Upload
                        </button>
                    </div>
                ...
                <button
                    ...
                    disabled={isLoading || (selectedTextIds.length === 0 && !selectedDatasetId)}
                    className="w-full px-4 py-2 rounded-md bg-orange-600 hover:bg-orange-700 disabled:opacity-50 text-white font-semibold mt-4"
                >
                        ...
                        <tbody>
                            {extractedRows.map((row, index) => (
                                <tr key={`${row.ref}-${index}`} className="border-t border-gray-100">
                                    <td className="p-3">
                                        {row.textSlug ? (
                                            <Link href={`/app/reader/${row.textSlug}`} className="text-orange-600 hover:underline">
