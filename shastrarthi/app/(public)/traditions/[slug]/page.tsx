    ...
    return (
        <main className="min-h-screen bg-gradient-to-br from-white via-orange-50/30 to-orange-100/50 py-20 px-4">
            <div className="container mx-auto max-w-5xl">
                <Link href="/traditions" className="text-sm text-orange-600 hover:text-orange-700 mb-8 inline-block">
                    ← All Traditions
                </Link>
                ...
                {relatedTexts.length > 0 ? (
                    <section className="mb-16">
                        <h2 className="text-2xl font-bold text-gray-900 mb-6">Foundational Texts</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {relatedTexts.map((text) => (
                                <Link
                                    key={text.id}
                                    href={`/app/reader/${text.slug}`}
                                    className="group bg-white rounded-xl p-6 border border-gray-200 hover:border-orange-300 transition-all shadow-sm hover:shadow-md"
                                >
                                    <h3 className="font-serif text-xl font-semibold text-gray-900 group-hover:text-orange-700 mb-2">
                                        {text.title_en}
                                    </h3>
                    ...
                    <div className="bg-orange-50 border border-orange-100 rounded-lg p-8 text-center mb-12">
                        <p className="text-orange-800 mb-4">Text digitization for this tradition is in progress.</p>
                        <Link href="/app/discover" className="btn-secondary inline-block">
                            Browse Library
                        </Link>
                    </div>
                )}
                ...
                <section>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <Link href="/app/discover" className="block p-6 bg-white border border-gray-200 rounded-xl hover:border-gray-300">
                            <h3 className="font-semibold text-gray-900 mb-2">Search Concepts</h3>
                            <p className="text-gray-600">Find specific concepts within {tradition.name} texts.</p>
                        </Link>
                        <Link href="/app/chat" className="block p-6 bg-white border border-gray-200 rounded-xl hover:border-gray-300">
                            <h3 className="font-semibold text-gray-900 mb-2">Ask a Master</h3>
                            <p className="text-gray-600">Chat with AI personas trained on {tradition.name} lineage.</p>
                        </Link>
                    </div>
                </section>
            </div>
