...
vi.mock("@/lib/learnlm", () => ({
    generateSynthesisResponse: (...args: unknown[]) => mockGenerateSynthesisResponse(...args),
    resolvePrompt: (id: string) => mockResolvePrompt(id),
    isConfigured: () => true,
}));
...
describe("POST /api/tools (simplify & translate, pSEO)", () => {
    ...
    it("returns 400 for unsupported mode", async () => {
        ...
        const response = await route.POST(
            makeJsonRequest("http://localhost/api/tools", "POST", {
                mode: "invalid_mode",
                payload: {},
            })
        );
